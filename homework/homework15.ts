...
const toCamelCase = (str: string): string => {
    let newWord: string = ''
    let splitWord: string[] = str.trim().toLowerCase().split(/\s+/)
    newWord += splitWord[0]
    if (splitWord.length === 1) return str
...
lowercase. */
const toSnakeCase = (str: string): string => str.trim().toLowerCase().replace(/\s+/g, '_')
...
ignored. */
const alternatingCases = (str: string): string => {
    let newWord: string = ''
...
const isNeutral = (str1: string, str2: string): string => {
    let newWord: string = ''
...
const isTrueOrFalse = (str: string): boolean => {
    let posCount: number = 0
    let negCount: number = 0
    const words = str.split(' ')
    for (let word of words) {
        const firstChar = word[0]
        if (!firstChar || !/[a-z]/i.test(firstChar)) continue
